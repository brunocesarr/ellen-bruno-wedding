...
export default async () => {
  ...
  console.log('[keep-alive] key prefix:', secretKey.slice(0, 12), 'len:', secretKey.length);
  ...
  try {
    ...
    console.error(
      ...
      'cause:', (e as any)?.cause?.code ?? (e as Error).message
    );
  ...
  const { error } = await supabase
    .from('keep_alive')
    .upsert({ id: 1, pinged_at: new Date().toISOString(), source: 'netlify' },
            { onConflict: 'id' });
