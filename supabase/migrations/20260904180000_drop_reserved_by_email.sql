-- reserved_by_email has been dead since reserve_gift was rewritten to its
-- current 5-arg signature (see 20260816120000_gift_pricing_kinds.sql /
-- 20260831140000_gift_payment_link.sql) — nothing has populated it since,
-- and no application code reads it anymore.

-- gifts_with_totals selects `g.*`, which freezes the column list at creation
-- time — must drop/recreate to drop the column. Both RPCs
-- `returns setof gifts_with_totals`, so Postgres ties their return type to
-- the view and blocks a bare drop; drop them first (same final signatures as
-- 20260831140000_gift_payment_link.sql) and recreate identically afterward —
-- bodies are unchanged, neither ever referenced reserved_by_email.
...
alter table public.gifts drop column if exists reserved_by_email;
