-- Tracks page views on the public gift detail page, per gift, for the admin
-- dashboard. Not guest-identifying (just a counter), so no PII/RLS concerns.

-- gifts_with_totals selects `g.*`, which freezes the column list at creation
-- time — must drop/recreate to add the column. Both RPCs
-- `returns setof gifts_with_totals`, so Postgres ties their return type to
-- the view and blocks a bare drop; drop them first (same signatures as
-- 20260904180000_drop_reserved_by_email.sql) and recreate identically
-- afterward — bodies are unchanged, neither references view_count.
...
alter table public.gifts add column view_count integer not null default 0;
...
-- Best-effort view counter for the public gift detail page. security definer
-- so anon can bump the counter without an UPDATE grant on gifts, which would
-- otherwise let anon write any column on the table.
create function public.increment_gift_view(p_gift_id uuid)
returns void
language plpgsql security definer set search_path = public as $$
begin
...
revoke all     on function public.increment_gift_view(uuid) from public;
grant  execute on function public.increment_gift_view(uuid) to anon, authenticated;
